...
impl Ledger {
    ...
    pub fn sum_match_counts(&self) -> usize {
        self.records
            .iter()
            .map(|record| {
                count_matches(record.chars.as_str().to_string(), record.groups.clone()).unwrap_or(0)
            })
            .sum()
    }
...
fn count_matches(input: String, groups: Vec<usize>) -> Option<usize> {
    ...
    if input.len() == 0 && groups.len() == 0 {
        return Some(1);
    }

    // The first group is the current group
    let grp_size = groups[0];
    // Remember the first broken spring's index
    let first_broken_spring_idx = input.find("#");
    ...
    for cindex in 0..=(input.len() - grp_size) {
        // Don't continue beyond the first broken spring's index
        if let Some(n) = first_broken_spring_idx {
            if cindex > n {
                break;
            }
        }
...
fn test_count_matches() {
    ...
    assert_eq!(count_matches(".?.?".to_string(), Vec::from([1])), Some(2));
    assert_eq!(
        count_matches(".#.....".to_string(), Vec::from([1])),
        Some(1)
    );
    assert_eq!(
        count_matches(".....#.".to_string(), Vec::from([1])),
    ...
    assert_eq!(
        count_matches("....###".to_string(), Vec::from([1, 3])),
        Some(0)
    );
    assert_eq!(
        count_matches("??.###".to_string(), Vec::from([1, 3])),
