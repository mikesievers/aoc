...
@pytest.mark.parametrize(
    "row,expected",
    [([1, 2, 2, 3], True), ([1, 4, 8, 5, 7], True)],
)
def test_s02p1_is_row_safe(row, expected):
    assert is_row_safe(row) == expected


@pytest.mark.parametrize("report_file,expected", [(SAMPLE_FILE, 4), (REPORT_FILE, 285)])
def test_count_safe_rows(report_file, expected):
    report = read_report(report_file)
    assert count_safe_rows(report, active_problem_dampener=True) == expected
